...
// Структура для описания части манипулятора
...
int main() {
	...
	// Загрузка 3D модели
	Model ourModel("models/Lab3 v.1.3.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (int i = 1; i < ourModel.meshes.size(); i++) {
			// Определение, к какой части манипулятора принадлежит текущий меш
			int partIndex = -1;
...
void initializeManipulatorParts() {
	...
	link2.rotationAxis = glm::vec3(0.0f, 1.0f, 0.0f);
	link2.pivotPoint = glm::vec3(-0.00133f, 0.0f, 0.16536f);
	...
	link3.rotationAxis = glm::vec3(0.0f, 1.0f, 0.0f);
...
// Вычисление матрицы преобразования для части манипулятора
glm::mat4 calculateTransformForPart(const ManipulatorPart& part) {
